...
func _gen_default_fight_func() -> void:
	var fight_func = func(unit1 : Unit, unit2 : Unit):
		...
		var tile_protection : int = _game_data_provider.get_grid().get_tile_vec(unit2.grid_position).protection
...
func initialize(game_data_provider, game_rules: Dictionary[String, Variant]) -> void:
	...
	# TODO This should be replaced with reading arguments from the game definition
