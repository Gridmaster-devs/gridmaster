...
#opens the save tactical map popup
#which will prompt a name to save with
func _open_save_tactical_popup() -> void: 
	var save_tact_map_popup: SaveTacticalMapPopup = preload("res://editor/editors/map_editor_refactor/popup_windows/save_tactical_map_popup.tscn").instantiate()
	save_tact_map_popup.tactical_map_saved.connect(_on_tactical_map_save)
	get_tree().call_group("map_editor_popup_manager", "add_new_popup", save_tact_map_popup, "save_tactical_map_popup")
	
#when save_tactical_popup is finished with a named tactical map
func _on_tactical_map_save(tactical_map_name: String) -> void: 
	_tactical_maps[tactical_map_name] = _tactical_painter.export_as_resource()
	...
	data[MapAttributes.STRATEGIC_TACTICAL_LIB_ITEM_ID] = tactical_map_name
	_strategic_painter.add_new_lib_item(data, MapAttributes.STRATEGIC_TACTICAL_LIB_NAME)
	get_tree().call_group("map_editor_popup_manager", "close_popup", "save_tactical_map_popup")
	_change_map_mode()
...
func _save_map():
	...
	_ftm.download_data(res, "MapResource.tres", "*.tres", true)
